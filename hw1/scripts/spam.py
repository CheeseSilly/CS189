...
if __name__=="__main__":
    data_mnist=np.load(f"../data/{"spam"}-data.npz")
    ...
    x_train_mnist,y_train_mnist,x_val_mnist,y_val_mnist=data_partition_spam(train_data,data_mnist["training_labels"])
    
    train_acc_list = []
    val_acc_list = []    
    
    model=svm.SVC(kernel="linear",C=100000,gamma=900)
